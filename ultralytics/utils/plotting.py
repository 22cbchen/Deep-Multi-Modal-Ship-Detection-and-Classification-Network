...
def plot_images(
    ...
    # Build Image
    mosaic = np.full((int(ns * h), int(ns * w), 4), 255, dtype=np.uint8)  # init # 修改
    for i in range(bs):
    ...
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        annotator.rectangle([x, y, x + w, y + h], None, (255, 255, 255, 255), width=2)  # borders   # 修改增加通道
        if paths:
            annotator.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(220, 220, 220, 220))  # filenames  # 修改增加通道
        if len(cls) > 0:
            idx = batch_idx == i
    ...
    # Save as RGBA image
    # Save as RGBA image
    if not save:    # 修改保存后的png图片
        return np.asarray(annotator.im.convert('RGBA'))  # Convert to RGBA before returning

    # Ensure the filename has a .png extension
    if not str(fname).lower().endswith('.png'):  # Convert fname to string and check extension
        fname = str(fname) + '.png'  # Append .png if not present

    # Save the image in RGBA format
    annotator.im.convert('RGBA').save(fname, format='PNG')  # save as RGBA in PNG format
    if on_plot:
        on_plot(fname)
